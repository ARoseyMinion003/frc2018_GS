...
//Red Portal2
	...
	function drawCubes(){
		...
		for (i=0; i< Red_Boost; i++){
			ctx.fillStyle = (Red_Boost_PU == pus.active)?"red":(Red_Boost_PU == pus.queued)?"rgba(255,0,0,"+coloro+")":(Red_Boost_PU == pus.done)?"black":"yellow";
			ctx.fillRect(15+(i*12),135,10,10);
		}
		for (i=0; i< Red_Levitate; i++){
//			ctx.fillStyle = Red_Levitate_PU?"red":"yellow";
			ctx.fillStyle = (Red_Levitate_PU == pus.active)?"red":(Red_Levitate_PU == pus.queued)?"rgba(255,0,0,"+coloro+")":(Red_Levitate_PU == pus.done)?"black":"yellow";
			ctx.fillRect(15+(i*12),148,10,10);
		}
		for (i=0; i< Red_Force; i++){
//			ctx.fillStyle = Red_Force_PU?"red":"yellow";
			ctx.fillStyle = (Red_Force_PU == pus.active)?"red":(Red_Force_PU == pus.queued)?"rgba(255,0,0,"+coloro+")":(Red_Force_PU == pus.done)?"black":"yellow";
			ctx.fillRect(15+(i*12),161,10,10);
		}
		for (i=0; i< Blu_Boost; i++){
//			ctx.fillStyle =Blu_Boost_PU?"blue":"yellow";
			ctx.fillStyle = (Blu_Boost_PU == pus.active)?"blue":(Blu_Boost_PU == pus.queued)?"rgba(0,0,255,"+coloro+")":(Blu_Boost_PU == pus.done)?"black":"yellow";
			ctx.fillRect(725-(i*12),161,10,10);
		}
		for (i=0; i< Blu_Levitate; i++){
//			ctx.fillStyle = Blu_Levitate_PU?"blue":"yellow";
			ctx.fillStyle = (Blu_Levitate_PU == pus.active)?"blue":(Blu_Levitate_PU == pus.queued)?"rgba(0,0,255,"+coloro+")":(Blu_Levitate_PU == pus.done)?"black":"yellow";
			ctx.fillRect(725-(i*12),148,10,10);
		}
		for (i=0; i< Blu_Force; i++){
//			ctx.fillStyle = Blu_Force_PU?"blue":"yellow";
			ctx.fillStyle = (Blu_Force_PU == pus.active)?"blue":(Blu_Force_PU == pus.queued)?"rgba(0,0,255,"+coloro+")":(Blu_Force_PU == pus.done)?"black":"yellow";
...
// Vault Labels
//		if (Red_Force_PU == pus.active) ctx.strokeStyle = "white"; else ctx.strokeStyle = "black";
		ctx.strokeText( "F", 5,171 );
//		if (Red_Levitate_PU == pus.active) ctx.strokeStyle = "white"; else ctx.strokeStyle = "black";
		ctx.strokeText( "L", 5,156 );
//		if (Red_Boost_PU == pus.active) ctx.strokeStyle = "white"; else ctx.strokeStyle = "black";
...
//Vault Labels
//		if (Blu_Boost_PU == pus.active) ctx.strokeStyle = "white"; else ctx.strokeStyle = "black";
		ctx.strokeText( "B", 742,171 );
//		if (Blu_Levitate_PU == pus.active) ctx.strokeStyle = "white"; else ctx.strokeStyle = "black";
		ctx.strokeText( "L", 742,156 );
//		if (Blu_Force_PU == pus.active) ctx.strokeStyle = "white"; else ctx.strokeStyle = "black";
		ctx.strokeText( "F", 742,141 );
	}

	function updateVaultScore(which){ //not working
		...
		//
		// RedScore += ((Red_Boost*5)+(Red_Levitate*5)+(Red_Force*5));
		// bluScore += ((Blu_Boost*5)+(Blu_Levitate*5)+(Blu_Force*5));
		if(which == "red"){
			RedScore += 5;
		}else if (which == "blue"){
			BlueScore += 5;
...
*/
	...
	// 	if (Red_Boost_PU == pus.queued && Red_Force_PU != pus.active && Blu_Force_PU != pus.active && Blu_Boost_PU != pus.active)
	// 	{currentPU = 1;	Red_Boost_PU = pus.active; }
	// 	else if (Blu_Boost_PU == pus.queued && Red_Force_PU != pus.active && Blu_Force_PU != pus.active && Red_Boost_PU != pus.active)
	// 	{currentPU = 4; Blu_Boost_PU = pus.active;}
	// 	else if (Red_Force_PU == pus.queued && Red_Boost_PU != pus.active && Blu_Force_PU != pus.active && Blu_Boost_PU != pus.active)
	// 	{currentPU = 3; Red_Force_PU = pus.active;} //3
	// 	else if (Blu_Force_PU == pus.queued && Red_Force_PU != pus.active && Blu_Boost_PU != pus.active && Red_Boost_PU != pus.active)
	// 	{currentPU = 6; Blu_Force_PU = pus.active;} //6
	// 	else if (Red_Levitate_PU == pus.active) {currentPU = 0; Red_Levitate_PU = pus.done;}
	// 	else if (Blu_Levitate_PU == pus.actvie) {currentPU = 0; Blu_Levitate_PU = pus.done;}
	// 	else currentPU = 0;
	// }
	function getNextPowerUp(){
			if (Red_Boost_PU == pus.queued && Red_Force_PU != pus.active && Blu_Force_PU != pus.active && Blu_Boost_PU != pus.active)
			...
			{currentPU = 6; Red_Force_PU = pus.active;} //3
			else if (Blu_Force_PU == pus.queued && Red_Force_PU != pus.active && Blu_Boost_PU != pus.active && Red_Boost_PU != pus.active)
			{currentPU = 3; Blu_Force_PU = pus.active;} //6
			else if (Red_Levitate_PU == pus.active) {currentPU = 0; Red_Levitate_PU = pus.done;}
			else if (Blu_Levitate_PU == pus.active) {currentPU = 0; Blu_Levitate_PU = pus.done;}
			else currentPU = 0;
	}
