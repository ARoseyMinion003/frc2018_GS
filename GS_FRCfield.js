...
//	var FE_IA_H	= 270;		// 1/10th of feet
	...
	function secondInterval(){
	  ...
	  RED_boost_K = Red_Boost_PU == pus.active?2:1;
		...
		if ((balanceSC > 10 || Blu_Force_PU == pus.active) && Red_Force_PU != pus.active) { //Blu_Force_PU
			if (scale > 0.5) {
		...
		if ((balanceSC < -10 || Red_Force_PU == pus.active) && Blu_Force_PU != pus.active) { //Red_Force_PU
			if (scale > 0.5) {
				RedScore += value * RED_boost_K;
				PtsSCRed += value * RED_boost_K;
				TimeSCRed++;
...
//		mouseloc1.innerHTML = "RedSW "+redswitch+"BlueSW "+blueswitch+"Scale "+scale;
	...
	function drawFE_Scale(){
		...
		if(Red_Force_PU == pus.active){
			ctx.strokeStyle = "Red";
			ctx.Style = "bold";
			ctx.strokeText("F",SCA_Left_X+(SC_W/2), (balanceSC*0.1)+centerline);
		}else if(Blu_Force_PU == pus.active){
			ctx.strokeStyle = "Blue";
			ctx.Style = "bold";
			ctx.strokeText("F",SCA_Left_X+(SC_W/2), (balanceSC*0.1)+centerline);
		}else if(Red_Boost_PU == pus.active){
			ctx.strokeStyle = "Red";
			ctx.Style = "bold";
			ctx.strokeText("B",SCA_Left_X+(SC_W/2), (balanceSC*0.1)+centerline);
		}else if(Blu_Boost_PU == pus.active){
			ctx.strokeStyle = "Blue";
			ctx.Style = "bold";
			ctx.strokeText("B",SCA_Left_X+(SC_W/2), (balanceSC*0.1)+centerline);
		}else {
			ctx.beginPath();
			ctx.arc(SCA_Left_X+(SC_W/2), (balanceSC*0.1)+centerline, 5, 0, 2*Math.PI ); //balance cirle
			ctx.fill();
			ctx.stroke();
		}
	...
	function drawFE_SwitchRed(){
		...
		if(Red_Force_PU == pus.active){
			ctx.strokeStyle = "Red";
			ctx.Style = "bold";
			ctx.strokeText("F",SWR_Left_X+(SW_W/2), (balanceSWR*0.1)+centerline);
		}else if(Red_Boost_PU == pus.active){
			ctx.strokeStyle = "Red";
			ctx.Style = "bold";
			ctx.strokeText("B",SWR_Left_X+(SW_W/2), (balanceSWR*0.1)+centerline);
		}else {
			ctx.beginPath();
			ctx.arc(SWR_Left_X+(SW_W/2), (balanceSWR*0.1)+centerline, 5, 0, 2*Math.PI ); //balance circle
			ctx.fill();
			ctx.stroke();
		}
	...
	function drawFE_SwitchBlue(){
		...
		if(Blu_Force_PU == pus.active){
			ctx.strokeStyle = "Blue";
			ctx.Style = "bold";
			ctx.strokeText("F",SWB_Left_X+(SW_W/2), (balanceSWB*0.1)+centerline);
		}else if(Blu_Boost_PU == pus.active){
			ctx.strokeStyle = "Blue";
			ctx.Style = "bold";
			ctx.strokeText("B",SWB_Left_X+(SW_W/2), (balanceSWB*0.1)+centerline);
		}else {
			ctx.beginPath();
			ctx.arc(SWB_Left_X+(SW_W/2), (balanceSWB*0.1)+centerline, 5, 0, 2*Math.PI ); //balance circle
			ctx.fill();
			ctx.stroke();
		}
