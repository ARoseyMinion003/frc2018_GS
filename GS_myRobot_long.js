...
/* *******   Simple Grafic Movements that work within C-space  ********** */
	...
	function moveSR(speed,rotate){
		...
		if (poseZ == 0){
			...
			else if ( (poseX > SCA_Base_X)&&(poseX < SCA_Base_X2)&&(poseY > SCA_Base_Y)&&(poseY < SCA_Base_Y2) )
			{
				poseX = prevX;
				poseY = prevY;
			} //robots need bumpers yo! //I don't think that this will work... :/
			// else if ( (poseX > (OposeX[0]-(robotW/2)))&&(poseY > (OposeY[0]-(robotW/2))) )
			// {
			// 	poseX = prevX;
			// 	poseY = prevY;
			// }

			// if(PointInRect(poseX,poseY,(OposeX[0]-12),(OposeY[0]-26),(robotW*1.5),(robotL*2.5))){ //2
			// 	poseX = prevX;
			// 	poseY = prevY;
			// }
	...
	function poopCube(){			// bacwards a = 0 or  //a
//		var dist = a?40:20;
		cubeX[theCube] = poseX-(40*Math.cos(poseR)); //dist
		cubeY[theCube] = poseY-(40*Math.sin(poseR)); //dist
		cubeR[theCube] = poseR;
