...
//			ctx.strokeText( "12", -10,-5);
	...
	function moveSR_opponent(speed,rotate,ix){
		...
		if (OposeZ[ix] == 0){
			...
			// if(PointInRect(OposeX[0],OposeY[0],(poseX-12),(poseY-26),(OrobotW*1.5),(OrobotL*2.5))){ //might need some tweaking
			// 	poseX = prevX;
			// 	poseY = prevY;
			// }
	...
	function poopCube_opponent(ix){ //needs to be tested
		cubeX[OtheCube[ix]] = OposeX[ix]-(40*Math.cos(OposeR[ix]));
		cubeY[OtheCube[ix]] = OposeY[ix]-(40*Math.sin(OposeR[ix]));
		// check to see if scored or on field
		cube[OtheCube[ix]] = 2;
